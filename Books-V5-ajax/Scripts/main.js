...
let Book = function (pTitle, pGenre, pYear, pAuthor) {
...
function RefreshList() {
    ...
    // need to insert api to wake up the server API path, using an "apicontroller"
    // need Book as this is a route, just like on Node where we put
    // Book routes in their own route section
    // the HTML is being server by the Home  MVC controller
    ...
    $.get("api/Book", function (data, status) {  // AJAX get
        ...
        books.forEach(function (item) {
            ...
            myli.innerHTML = item.BookId + ":  " + item.Title + ":  " + item.Genre + ":  " + item.Year + ": " + item.Author;
...
function updateItem() {
    ...
    let newBook = new Book(title, genre, year, Author);
    let searchId = document.getElementById("SearchId").value
